...
```{r}
# 202330BCEPR_stom
my_dat %>% filter(trial_name == "202337DMEPR_momi") %>% 
...
ggsave(paste0("images\\germ_momi", trial_interest, "_", ".png"),
        units = "in", dpi = 300, width = 15, height = 6
   )


# 202338CQEPR_repe
my_dat %>% filter(trial_name == "202336DMEPR_repe") %>% 
  ggplot(aes(x = reorder(accession_name, -germination_perc), 
             y = germination_perc, fill = factor(rep_number))) +
  facet_grid(factor(rep_number)~trial_name) +
  geom_col(position = "dodge") +
...
BLUEs_BLUPs <- BLUEs_BLUPs %>% 
  select(genotype, contains("DM_gravity"), 
         everything())
...
```{r}
res.pca <- pca_index(data = index_dat, id = "accession_name", b = c(10, 10, 10, 15), percentage = 0.15)
...
accessions <- res.pca$selection %>% 
  filter(germination_perc >= 50) %>% 
  arrange(desc(DM_gravity)) %>% row.names()

selections <- res.pca$results %>% rownames_to_column(var = "accession_name") %>% 
  mutate(selected = ifelse(accession_name %in% accessions, TRUE, FALSE)) %>% arrange(desc(germination_perc))
