...
```{r}
# 202330BCEPR_stom
my_dat %>% filter(trial_name == "202359DVEPR_momi") %>% 
...
# 202338CQEPR_repe
my_dat %>% filter(trial_name == "202358EPR_repe") %>% 
  ggplot(aes(x = reorder(accession_name, -germination_perc), 
             y = germination_perc, fill = factor(rep_number))) +
  facet_grid(factor(rep_number)~trial_name) +
  geom_col(position = "dodge") +
...
BLUPs_momi <- master_data$BLUP_BLUE_202359DVEPR_momi %>% 
  select(genotype, starts_with("BLUPs"))

colnames(BLUPs_momi)[-1] <- paste(colnames(BLUPs_momi)[-1], "202359DVEPR_momi", sep = "_")

BLUEs_BLUPs <- BLUEs_BLUPs %>% 
  select(-BLUEs_germination_perc_202358DVEPR_repe) %>% left_join(
   BLUPs_momi 
  )

master_data[["BLUEs_BLUPs_MET"]] = BLUEs_BLUPs

master_data[["BLUPs_momi"]] = BLUPs_momi
```
...
geno_cor <- gg_cor(
  colours = c("red", "white", "blue"),
  data = BLUPs_momi, # remove carotenoids
  label_size = 3
) + 
  theme(
...
colnames(blupDF_kp)[-1] <- str_remove(colnames(blupDF_kp)[-1], "_202359DVEPR_momi")
